...
public class RegistrationController {
  ...
  public RegistrationController(PremiumUserService premiumUserService) {
  ...
  public String processRegistration(@Valid @ModelAttribute("registrationRequest") RegistrationRequest registrationRequest,
    ...
    try {
      ...
      PremiumUserDTO userDto = convertToUserDto(registrationRequest);
      ...
      logger.error("Registration failed for user: {} - duplicate resource", registrationRequest.getUsername());
      model.addAttribute("errorMessage",
          "Username or email already exists. Please choose different credentials.");
      return "register";

    } catch (ValidationException e) {
      logger.error("Registration failed for user: {} - validation error", registrationRequest.getUsername());
      model.addAttribute("errorMessage",
  ...
  // Helper method to convert RegistrationRequest to PremiumUserDTO
  private PremiumUserDTO convertToUserDto(RegistrationRequest request) {
    return new PremiumUserDTO(
        ...
        request.getEmail(),
        request.getMobile(),
        request.getDiveCertification(),
        request.getUsername(),
        request.getPassword(),
        "ROLE_PREMIUM", // default role
        LocalDateTime.now(),
        LocalDateTime.now()
    );
  }
