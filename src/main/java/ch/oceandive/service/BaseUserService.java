...
public abstract class BaseUserService<T, D, ID> {
  ...
  // Validate basic user fields for UPDATE operations (password optional)
  protected void validateBasicUserFieldsForUpdate(String email, String username,
    ...
    // Note: Password is NOT validated here - it's optional for updates
  }
  ...
  protected D updateUser(ID id, D dto, String entityType) {
    ...
    // Validate user fields for UPDATE (password optional)
  ...
  protected <V> void updateIfNotNull(V newValue, BiConsumer<T, V> setter, T entity) {
    if (newValue != null) {
      setter.accept(entity, newValue);
    }
  }

  // Helper method to update password if not empty
  protected void updatePasswordIfNotEmpty(String password, BiConsumer<T, String> setter, T entity) {
    if (password != null && !password.isEmpty()) {
      setter.accept(entity, encodePassword(password));
    }
  }
