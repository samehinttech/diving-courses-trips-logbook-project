...
public class CourseService {
    ...
    // Get courses with a start date after the given date.
    ...
    // Get courses with a start date before the given date.
    public List<Course> getCoursesByStartDateBefore(LocalDate date) {
     ...
     // Get courses with a start date between the given dates.
    ...
    public boolean canEnrollInCourse(Long courseId, DiveCertification userCertification) {
     ...
     // Book a course if the user has adequate certification.
    @Transactional
    ...
    public List<Course> getFeaturedCourses(int limit) {
        ...
        return courseRepo.findByStatusAndFeaturedOrderByDisplayOrderAsc(
            PublicationStatus.PUBLISHED, true, pageable).getContent();
    }

    // Get course by slug for SEO-friendly URLs.
    public Optional<Course> getCourseBySlug(String slug) {
        return courseRepo.findBySlugAndStatus(slug, PublicationStatus.PUBLISHED);
    }
   //  Get available courses (published, not full, future dates).
  public List<Course> getAvailableCourses() {
        return courseRepo.findAvailableCoursesList(PublicationStatus.PUBLISHED, LocalDate.now());
    }

    // Get available courses with pagination.
    public Page<Course> getAvailableCourses(Pageable pageable) {
        return courseRepo.findAvailableCoursesPage(PublicationStatus.PUBLISHED, LocalDate.now(), pageable);
    }
     // Get upcoming courses (regardless of booking status).
    public List<Course> getUpcomingCourses() {
        return courseRepo.findUpcomingCourses(PublicationStatus.PUBLISHED, LocalDate.now());
    }
     // Search published courses.
    public Page<Course> searchPublishedCourses(String searchTerm, Pageable pageable) {
        return courseRepo.searchCourses(searchTerm, PublicationStatus.PUBLISHED, pageable);
    }
    ...
    // Create a course
    @Transactional
    public Course adminCreateCourse(Course course) {
...
// Update an existing course by ID.
    @Transactional
    public Course adminUpdateCourse(Long id, Course courseDetails) {
    ...
    // Toggle featured status of a course for the admin dashboard.
    @Transactional
   ...
   //  Change course status.
    @Transactional
    public Course changeStatus(Long courseId, PublicationStatus status) {
        Course course = getCourseById(courseId);
        PublicationStatus oldStatus = course.getStatus();
    ...
    public long getPublishedCourseCount() {
        return courseRepo.countByStatus(PublicationStatus.PUBLISHED);
    }

    public long getAvailableSpotsCount() {
        Long spots = courseRepo.countAvailableSpots(PublicationStatus.PUBLISHED, LocalDate.now());
